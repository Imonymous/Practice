...
def add_nodes(a, left, right):
	if left > right:
		return None
	mid_idx = (left+right)/2
	root = TreeNode(a[mid_idx])
	root.left_ptr = add_nodes(a, left, mid_idx-1)
	root.right_ptr = add_nodes(a, mid_idx+1, right)
	return root

def build_balanced_bst(a):
    root = add_nodes(a, 0, len(a)-1)
    return root
